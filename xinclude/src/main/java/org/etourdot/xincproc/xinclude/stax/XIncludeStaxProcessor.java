...
class XIncludeStaxProcessor
{
    ...
    private void readingStreamWithXPointer(final XMLEventReader reader, final URI baseRelativeURI,
                                           final Attribute parentLangAttribute,
        ...
        while (reader.hasNext())
        {
            ...
            if (type == XMLStreamConstants.START_ELEMENT)
            {
                ...
                topLangAttribute = readingStartElementWithXPointer(startElement, level, baseRelativeURI,
                            parentLangAttribute, topLangAttribute, writer, factory.getConfiguration().isLanguageFixup());
                level ++;
            }
            else if (type == XMLStreamConstants.END_ELEMENT)
            {
                final EndElement endElement = event.asEndElement();
                if (XPointerHelper.isTopElement(endElement.getName()))
                {
                    topLangAttribute = null;
                }
                else if (!XPointerHelper.isRootElement(endElement.getName()))
                {
                    writer.add(event);
                }
                level --;
    ...
    private Attribute readingStartElementWithXPointer(final StartElement startElement, final int level, final URI baseRelativeURI,
            ...
            throws Exception
    {
        if (XPointerHelper.isTopElement(startElement.getName()))
        {
            return getTopLangAttribute(startElement);
        }
        else if (!XPointerHelper.isRootElement(startElement.getName()))
        {
    ...
    private byte[] getXIncludeBytes(final ByteArrayOutputStream baOutputStream)
            throws Exception
    {
        ...
        //XPointerEngine xPointerEngine = factory.getConfiguration().getXPointerEngine();
        //xPointerEngine.parse(in, out);

...
public class XIncludeFilter extends XMLFilterImpl
{
                             ...
                             final Attributes attributes) throws SAXException
    {
        ...
        if (XPointer.isTopElement(qnameElt))
        {

        }
        else if (!XPointer.isRootElement(qnameElt))
        {
            ...
            throws XIncludeFatalException, XIncludeResourceException, XIncludeDocumentNotFoundException
    {
        try {
            ...
            else
            {
                ...
                final XPointerContext xPointerContext = new XPointerContext(xpointer, configuration);
                for (final Entry<String, String> namespace : namespaces.entrySet())
                {
                    xPointerContext.addPrefix(namespace.getKey(), namespace.getValue());
                }
                final XPointer pointer = configuration.getXPointerEngine().parse(xPointerContext);
                final XMLReader parser = XMLReaderFactory.createXMLReader();
                parser.setContentHandler(this);
                parser.setEntityResolver(new XIncludeFilter.SimpleEntityResolver());
                final StringWriter writer = new StringWriter();
                pointer.parse(new StringReader(new XdmNode(doc).toString()), writer);
        ...
        catch (final RecognitionException ignored)
        {
            throw new XIncludeFatalException("Recognition problem");
        }
